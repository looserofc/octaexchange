...
const ADMIN_EMAIL = "admin@nextrade.com", ADMIN_PASS = "admin123";
const TAKEN = new Set(["admin","nextrade","user1","test"]);
...
export default function AuthPage() {
  ...
  const { setUser, setAdminAuthed, setShowAdmin, addToast } = useStore();
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      // Admin login
      if (tab === "login" && email === ADMIN_EMAIL && pw === ADMIN_PASS) {
        setAdminAuthed(true);
        setShowAdmin(true);
      ...
      setUser({
        id: "u_" + Date.now(),
        username: tab === "signup" ? uname : "alexmorgan",
        name,
        email,
        phone: tab === "signup" ? phone : "",
        avatar: initials(name),
        fundingBalance: 1247.85,
        tradingBalance: 200,
        tradingFreezeUntil: Date.now() + 15 * 24 * 60 * 60 * 1000,
        tier: TIERS[1],
        totalProfit: 84.20,
        totalTrades: 42,
        subscribedTier: null,
        pendingTier: null,
        kycStatus: null,
        referralCode: "NXT" + (Date.now() + "").slice(-5),
        referralCount: 0,
        referrals: [],
        referredBy: ref || null,
        joinDate: new Date().toISOString().split("T")[0],
      });
      addToast(tab === "signup" ? "Welcome to NexTrade! 🎉" : "Welcome back!", "ok");
      setBusy(false);
    }, 900);
  };
    ...
    <div className="auth-shell">
      ...
      <div style={{ flex:1, padding:"0 20px 40px", maxWidth:440, margin:"0 auto", width:"100%" }}>
        ...
        <div style={{ display:"flex", borderBottom:"1px solid var(--ln)", marginBottom:24 }}>
          {["login","signup"].map(t => (
            <button key={t} className={`auth-tab${tab===t?" on":""}`} onClick={() => { setTab(t); setErrs({}); }}>
        ...
        {tab === "signup" && <>
          <div className="fg">
            <label className="lbl">Username <span style={{ color:"var(--dn)" }}>*</span></label>
            <div className="iw">
              <input className="inp" placeholder="e.g. johndoe99" value={uname}
                onChange={e => { setUname(e.target.value); ce("uname"); }} onKeyDown={K}
                style={{ borderColor:errs.uname?"var(--dn)":"" }}/>
              {uname && !errs.uname && <span style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", color:"var(--up)" }}>✓</span>}
            </div>
            <Er msg={errs.uname}/>
          </div>

          <div className="fg">
            <label className="lbl">Email <span style={{ color:"var(--dn)" }}>*</span></label>
            <input className="inp" type="email" placeholder="you@email.com" value={email}
              onChange={e => { setEmail(e.target.value); ce("email"); }} onKeyDown={K}
              style={{ borderColor:errs.email?"var(--dn)":"" }}/>
            <Er msg={errs.email}/>
          </div>

          <div className="fg">
            <label className="lbl">Phone <span style={{ color:"var(--t3)", fontSize:10, marginLeft:4 }}>(optional)</span></label>
            <input className="inp" type="tel" placeholder="+1-555-0000" value={phone}
              onChange={e => setPhone(e.target.value)} onKeyDown={K}/>
          </div>

          <div className="fg">
            <label className="lbl">Create Password <span style={{ color:"var(--dn)" }}>*</span></label>
            <div className="iw">
              <input className="inp" type={sp?"text":"password"} placeholder="Min. 8 characters" value={pw}
                onChange={e => { setPw(e.target.value); ce("pw"); }} onKeyDown={K}
                style={{ paddingRight:44, borderColor:errs.pw?"var(--dn)":"" }}/>
              <Eye show={sp} toggle={() => setSp(p => !p)}/>
            </div>
            {pw.length > 0 && (
          ...
          <div className="fg">
            ...
            <div className="iw">
              <input className="inp" type={scp?"text":"password"} placeholder="Repeat password" value={cpw}
                onChange={e => { setCpw(e.target.value); ce("cpw"); }} onKeyDown={K}
                style={{ paddingRight:44, borderColor:errs.cpw?"var(--dn)":cpw&&cpw===pw?"var(--up)":"" }}/>
              <Eye show={scp} toggle={() => setScp(p => !p)}/>
            </div>
            <Er msg={errs.cpw}/>
          </div>

          <div className="fg">
            <label className="lbl">Referral Code <span style={{ color:"var(--t3)", fontSize:10, marginLeft:4 }}>(optional)</span></label>
            <input className="inp" placeholder="Enter code if you have one" value={ref}
              onChange={e => setRef(e.target.value)} onKeyDown={K}/>
          </div>

          <div style={{ marginBottom:20 }}>
            <div style={{ display:"flex", alignItems:"flex-start", gap:10, cursor:"pointer" }}
              onClick={() => { setTerms(p => !p); ce("terms"); }}>
              <div style={{ width:20, height:20, borderRadius:5, flexShrink:0, marginTop:2,
                border:`2px solid ${errs.terms?"var(--dn)":terms?"var(--gold)":"var(--ln2)"}`,
              ...
              <span style={{ fontSize:13, color:"var(--t2)", lineHeight:1.5 }}>
                I agree to the <span style={{ color:"var(--gold)", fontWeight:600 }}>Terms & Conditions</span> and <span style={{ color:"var(--gold)", fontWeight:600 }}>Privacy Policy</span>
              </span>
            </div>
            <Er msg={errs.terms}/>
          </div>
        ...
        {tab === "login" && <>
          <div className="fg">
            ...
            <input className="inp" type="email" placeholder="you@email.com" value={email}
              onChange={e => { setEmail(e.target.value); ce("email"); }} onKeyDown={K}
              style={{ borderColor:errs.email?"var(--dn)":"" }} autoComplete="email"/>
            <Er msg={errs.email}/>
          </div>

          <div className="fg">
            <label className="lbl">Password <span style={{ color:"var(--dn)" }}>*</span></label>
            <div className="iw">
              <input className="inp" type={sp?"text":"password"} placeholder="Your password" value={pw}
                onChange={e => { setPw(e.target.value); ce("pw"); }} onKeyDown={K}
                style={{ paddingRight:44, borderColor:errs.pw?"var(--dn)":"" }} autoComplete="current-password"/>
              <Eye show={sp} toggle={() => setSp(p => !p)}/>
            </div>
            <Er msg={errs.pw}/>
          </div>
          ...
          <div style={{ textAlign:"right", marginTop:-8, marginBottom:16 }}>
            <span
              style={{ fontSize:12, color:"var(--gold)", cursor:"pointer", fontWeight:600 }}
          ...
          <div style={{ background:"rgba(45,156,255,.06)", border:"1px solid rgba(45,156,255,.12)", borderRadius:10, padding:"10px 14px", marginBottom:16 }}>
            ...
            <div style={{ fontSize:11, color:"var(--t2)", fontFamily:"var(--m)", lineHeight:1.8 }}>
              User:&nbsp; any@email.com / anypassword<br/>
        ...
        <button className="btn btn-gold btn-block btn-lg" onClick={submit} disabled={busy} style={{ marginBottom:14 }}>
          {busy ? <span className="spin"/> : tab === "login" ? "Log In" : "Create Account"}
        </button>
