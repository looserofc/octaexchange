...
export default function TradingViewChart({ sym = "BTC", interval = "60", height = 380 }) {
  ...
  useEffect(() => {
    ...
    // Small delay to ensure DOM is clean before mounting new widget
    const timer = setTimeout(() => {
      const currentContainer = containerRef.current;
      if (!currentContainer) return;

      // Create wrapper div — set ID BEFORE appending to DOM
      ...
      // Append wrapper first so DOM element exists when script runs
    ...
    }, 50); // 50ms delay — enough for React to finish DOM updates
